...
public class MainController {
	//this is set collections for storing player1 ships after dragging and droping
	...
	//this object for storing player1 details like ships sunk, hits, missed and ships hit
	@Autowired
	Player1 player1;
	//this object for storing player2 details like ships sunk, hits, missed and ships hit
	@Autowired
	Player2 player2;
	//This object stores sizes with names of all five ships for using if all blocks of a ship have been hit
	// and then storing them into sunk ship collections
	@Autowired
	ShipSizes shipSizes;
	//this Api end point listen call on move of player1 for storing data about move like hit
	...
	public void updatePlayer1(@RequestBody DataDao dataDao) {
		...
		if(!dataDao.getShip().equalsIgnoreCase("none")) {
			...
			List<String> shipList=player1.getPlayerHits()
					.stream().filter(sh->sh.equalsIgnoreCase(dataDao.getShip())).toList();
			if(shipList.size()==shipSizes.getShipSizes().get(dataDao.getShip())) {
				...
				List<String> newList=player1.getPlayerHits()
						.stream().filter(sh->!(sh.equalsIgnoreCase(dataDao.getShip()))).toList();
	...
	//this Api end point listen call on move of player2 for storing data about move like hit
	// missed ship name is a ship sunk and then storing in sunk list
	...
	public void updatePlayer2(@RequestBody DataDao dataDao) {
		...
		if(!dataDao.getShip().equalsIgnoreCase("none")) {
			...
			List<String> shipList=player2.getPlayerHits()
					.stream().filter(sh->sh.equalsIgnoreCase(dataDao.getShip())).toList();
			if(shipList.size()==shipSizes.getShipSizes().get(dataDao.getShip())) {
				...
				List<String> newList=player2.getPlayerHits()
						.stream().filter(sh->!(sh.equalsIgnoreCase(dataDao.getShip()))).toList();
	...
	// sunk ship size are 5 then player1 returned as wining otherwise no win 
	...
	// This Api is called after every move of player2 to view the result and see if
		// sunk ship size are 5 then player2 returned as wining otherwise no win 
	...
	// This API is called after dragging of each ship by player1 to store ships into Set
	...
	// This API is called after dragging of each ship by player2 to store ships into Set
	...
	// This end point is called on start button pressed by player2 and return set of detail about
	...
	// This end point is called on start button pressed by player1 and return set of detail about
	...
	// This APi end point is called on clicking restart button for refreshing everything
	@GetMapping("refresh")
	public void refresh() {
